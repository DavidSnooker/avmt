...
/*
   2006/06/27 - Added ksocket_send, so, after receive a packet, 
   the kernel send another back to the CONNECT_PORT
...
static void ksocket_start(void) {
	...
	/* create a socket */
	if (((err = sock_create(AF_INET, SOCK_DGRAM, 
		IPPROTO_UDP, &kthread->sock)) < 0) 
		|| ((err = sock_create(AF_INET, SOCK_DGRAM, 
		IPPROTO_UDP, &kthread->sock_send)) < 0)) {
	...
	if (((err = kthread->sock->ops->bind(kthread->sock, 
		...
		printk(KERN_INFO MODULE_NAME
			": Could not bind or connect to socket, error = %d\n", -err);
		goto close_and_out;
	}
...
void __exit ksocket_exit(void) {
	...
	if (kthread->thread==NULL) {
		...
		send_sig(SIGTERM, kthread->thread, 1);
...
/* module information */
MODULE_DESCRIPTION("kernel thread listening on a UDP socket");
